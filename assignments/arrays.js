...
let carModels = [];
//'riolet was lowercase and missed the sorting
//Turning all first letters uppercase, then sorting and logging to console.
for(let i = 0; i < inventory.length; i++){
    carModels[i] = inventory[i].car_model; 
    let model = carModels[i].split('');  
    model.splice(0,1,model[0].toUpperCase());   
    carModels.splice(i,1,model.join(''));
}
carModels.join('');
carModels.sort();
...
console.log(`\nChallenge 5 Output:\nThere are ${oldCars.length} cars older than the year 2000 on your lot.`);
...
for(let i = 0; i < inventory.length; i++){
    if(inventory[i].car_make == "Audi" || inventory.car_make == "BMW"){
